...
class CountBenchmarkUser(MongoUser):
    ...
    def _seed_and_index(self):
        ...
        seed_count_collection(
            ...
            self.seed_docs,
            self.document_size,
            generate_fn=self.generate_document,
        )
...
def seed_count_collection(
    ...
    generate_fn: Optional[Callable[[int], dict]] = None,
) -> int:
    """Seed a collection with documents for aggregation benchmarks.

    Documents are generated by the supplied *generate_fn* (or the default
    ``"standard"`` generator) and inserted in batches using
    ...
    Args:
        ...
        generate_fn: A ``generate_document(size_bytes) -> dict`` callable.
            Defaults to the ``"standard"`` generator if not provided.

    Returns:
        The total number of documents inserted.
    """
    if generate_fn is None:
        generate_fn = get_generator("standard")
    ...
    for batch_idx in range(total_batches):
        ...
        batch = [generate_fn(document_size) for _ in range(current_batch_size)]
